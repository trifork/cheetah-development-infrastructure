...
for topic in _kafkasql-journal $KAFKA_TOPICS; do
	...
	bash bin/kafka-configs.sh --bootstrap-server kafka:19093 --entity-type topics --entity-name "$topic" --alter --add-config retention.ms=94608000000 --add-config cleanup.policy=compact
done
